...
export const getAllVendasByData = async (req: Request, res: Response) => {
  try {
    ...
    const formasPagamentoIds = formasPagamento
      ? (formasPagamento as string).split(",").map(Number)
      : undefined;

    // Prepara o filtro de status
    const statusList = status ? (status as string).split(",") : undefined;
    ...
    const totais = {
      ...
      porFormaPagamento: vendas.reduce((acc: any, venda) => {
      ...
      porStatus: vendas.reduce((acc: any, venda) => {
