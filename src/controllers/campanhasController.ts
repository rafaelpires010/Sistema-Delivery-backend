...
import { createCampanhaSchema, updateCampanhaSchema } from "../schema/Campanha";

const prisma = new PrismaClient();

export const getCampanhasByTenant: RequestHandler = async (
  req: Request,
  res: Response
) => {
  try {
...
export const getCampanhaById: RequestHandler = async (
  req: Request,
  res: Response
) => {
  try {
...
//criar campanha
export const createCampanha: RequestHandler = async (
  req: Request,
  res: Response
) => {
  try {
    const { nome, cupomId, descricao } = req.body;
    const img = (req.file as any)?.location; // URL do S3 fornecida pelo multer-s3
    const tenantSlug = req.params.tenantSlug;
    ...
    const body = createCampanhaSchema.safeParse(data);
    if (!body.success) {
      return res
        .status(400)
...
//deletar campanha
export const deleteCampanha: RequestHandler = async (
  req: Request,
  res: Response
) => {
...
//editar campanha
export const updateCampanha: RequestHandler = async (
  req: Request,
  res: Response
) => {
  ...
  try {
    ...
    const data = {
      ...
      cupomId: req.body.cupomId
        ? parseInt(req.body.cupomId as string, 10)
        : campanha.cupomId,
      ativo: req.body.ativo ?? campanha.ativo,
    };

    // Validar os dados com o schema
    const body = updateCampanhaSchema.safeParse(data);
    if (!body.success) {
      return res
        .status(400)
        .json({ error: "Dados inválidos", details: body.error.issues });
