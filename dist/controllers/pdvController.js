...
exports.reimprimirCupomEspecifico = exports.reimprimirUltimoCupom = exports.cancelarUltimaVendaPdv = exports.cancelarVendaPdv = exports.trocarOperador = exports.registrarOrderAndVendaPdv = exports.getAllPdvs = exports.getCaixaAtual = exports.registrarSuprimento = exports.registrarSangria = exports.fecharCaixa = exports.abrirCaixa = exports.createPdv = void 0;
const client_1 = require("@prisma/client");
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const bcrypt_1 = __importDefault(require("bcrypt"));
const prisma = new client_1.PrismaClient();
// Criar um novo PDV
const createPdv = async (req, res) => {
    try {
...
const abrirCaixa = async (req, res) => {
    try {
        ...
        if (ultimaOperacao && ultimaOperacao.dataAbertura) {
            ...
            const hoje = new Date();
            // Se for do mesmo dia
            if (dataAbertura.getDate() === hoje.getDate() &&
            ...
            return res.status(400).json({
                ...
                message: "Existe um caixa aberto do dia anterior que precisa ser fechado",
                dataAbertura: dataAbertura,
            });
        }
    ...
    try {
        ...
        if (!aberturaCaixa) {
            return res.status(400).json({
                ...
                message: "Para realizar operações, é necessário abrir o caixa novamente",
            });
        }
        if (aberturaCaixa && aberturaCaixa.dataAbertura) {
            ...
            if (dataAbertura.getDate() < hoje.getDate() ||
                ...
                return res.status(400).json({
                    error: "Caixa com fechamento pendente",
                    message: "O caixa foi aberto em um dia anterior e precisa ser fechado. Após o fechamento, abra o caixa novamente.",
                    dataAbertura: dataAbertura,
                });
            }
        }
        ...
        const totalSangrias = sangrias.reduce((total, sangria) => total + sangria.valor, 0);
        const totalSuprimentos = suprimentos.reduce((total, suprimento) => total + suprimento.valor, 0);
        // Atualiza o cálculo do valor final
        const valorFinal = (aberturaCaixa.valorInicial ?? 0) +
            valorTotalVendas +
            totalSuprimentos -
            totalSangrias;
...
exports.fecharCaixa = fecharCaixa;
...
exports.registrarSangria = registrarSangria;
// Registrar um suprimento
const registrarSuprimento = async (req, res) => {
    try {
        const { tenantSlug } = req.params;
        const { pdvId, operadorId, operadorSenha, valor, motivo } = req.body;
        if (!pdvId || !operadorId || !operadorSenha || !valor) {
...
exports.registrarSuprimento = registrarSuprimento;
...
exports.getCaixaAtual = getCaixaAtual;
const getAllPdvs = async (req, res) => {
    try {
        const { tenantSlug } = req.params;
        const tenant = await prisma.tenant.findUnique({
            where: { slug: tenantSlug },
...
exports.getAllPdvs = getAllPdvs;
const registrarOrderAndVendaPdv = async (req, res) => {
    try {
        const { tenantSlug } = req.params;
        const { pdvId, operadorId, operadorSenha, produtos, valor, formaPagamentoId, } = req.body;
        if (!pdvId ||
            !operadorId ||
            !operadorSenha ||
            !produtos ||
...
exports.registrarOrderAndVendaPdv = registrarOrderAndVendaPdv;
const trocarOperador = async (req, res) => {
    try {
        const { tenantSlug } = req.params;
        const { pdvId, operadorId, operadorSenha } = req.body;
        if (!pdvId || !operadorId || !operadorSenha) {
        ...
        const senhaCorreta = novoOperador.operadorSenha.startsWith("$2b$")
            ? await bcrypt_1.default.compare(operadorSenha, novoOperador.operadorSenha)
...
exports.trocarOperador = trocarOperador;
const cancelarVendaPdv = async (req, res) => {
    try {
        const { tenantSlug } = req.params;
        ...
        if (!pdvId || !operadorId || !operadorSenha || !nrVenda) {
            return res.status(400).json({
                error: "Dados inválidos",
                message: "PDV, operador, senha, número da venda e motivo são obrigatórios",
            });
        }
        const tenant = await prisma.tenant.findUnique({
            where: { slug: tenantSlug },
        ...
        if (!venda) {
            return res.status(404).json({
                ...
                message: "Verifique se o número da venda está correto e se ela ainda pode ser cancelada",
            });
        }
...
exports.cancelarVendaPdv = cancelarVendaPdv;
const cancelarUltimaVendaPdv = async (req, res) => {
    try {
        const { tenantSlug } = req.params;
        const { pdvId, operadorId, operadorSenha } = req.body;
        if (!pdvId || !operadorId || !operadorSenha) {
...
exports.cancelarUltimaVendaPdv = cancelarUltimaVendaPdv;
const reimprimirUltimoCupom = async (req, res) => {
    try {
        const { tenantSlug } = req.params;
        const { pdvId, operadorId, operadorSenha } = req.body;
        if (!pdvId || !operadorId || !operadorSenha) {
...
exports.reimprimirUltimoCupom = reimprimirUltimoCupom;
const reimprimirCupomEspecifico = async (req, res) => {
    try {
        const { tenantSlug } = req.params;
        const { pdvId, operadorId, operadorSenha, nrVenda } = req.body;
        if (!pdvId || !operadorId || !operadorSenha || !nrVenda) {
...
exports.reimprimirCupomEspecifico = reimprimirCupomEspecifico;
