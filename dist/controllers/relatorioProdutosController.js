...
exports.getAllProdutosByData = void 0;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
const getAllProdutosByData = async (req, res) => {
    try {
        ...
        const categoriaIds = categoriaId
            ? categoriaId.split(",").map(Number)
            : undefined;
        const produtoIds = produtoId
            ? produtoId.split(",").map(Number)
            : undefined;
        // Monta a query base
        const where = {
            id_tenant: tenant.id,
...
exports.getAllProdutosByData = getAllProdutosByData;
